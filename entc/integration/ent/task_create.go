...
// Save upsert the Task in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *TaskUpsertOne) Save(ctx context.Context) (*Task, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: TaskUpsertOne.Save is not supported by MySQL driver. Use TaskUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *TaskUpsertOne) SaveX(ctx context.Context) *Task {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
