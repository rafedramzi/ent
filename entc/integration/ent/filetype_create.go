...
// Save upsert the FileType in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *FileTypeUpsertOne) Save(ctx context.Context) (*FileType, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: FileTypeUpsertOne.Save is not supported by MySQL driver. Use FileTypeUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *FileTypeUpsertOne) SaveX(ctx context.Context) *FileType {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
