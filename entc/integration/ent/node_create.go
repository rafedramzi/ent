...
// Save upsert the Node in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *NodeUpsertOne) Save(ctx context.Context) (*Node, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: NodeUpsertOne.Save is not supported by MySQL driver. Use NodeUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *NodeUpsertOne) SaveX(ctx context.Context) *Node {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
