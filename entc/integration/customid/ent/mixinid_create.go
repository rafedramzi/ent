...
// Save upsert the MixinID in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *MixinIDUpsertOne) Save(ctx context.Context) (*MixinID, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		// fields from the database since MySQL does not support the RETURNING clause.
		return nil, errors.New("ent: MixinIDUpsertOne.Save is not supported by MySQL driver. Use MixinIDUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *MixinIDUpsertOne) SaveX(ctx context.Context) *MixinID {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
