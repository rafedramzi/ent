...
// Save upsert the Link in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *LinkUpsertOne) Save(ctx context.Context) (*Link, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		// In case of "ON CONFLICT", there is no way to get back the record
		// fields from the database since MySQL does not support the RETURNING clause.
		return nil, errors.New("ent: LinkUpsertOne.Save is not supported by MySQL driver. Use LinkUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *LinkUpsertOne) SaveX(ctx context.Context) *Link {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
