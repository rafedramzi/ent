...
// Save upsert the IntSID in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *IntSIDUpsertOne) Save(ctx context.Context) (*IntSID, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: IntSIDUpsertOne.Save is not supported by MySQL driver. Use IntSIDUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *IntSIDUpsertOne) SaveX(ctx context.Context) *IntSID {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
