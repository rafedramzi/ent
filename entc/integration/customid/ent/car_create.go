...
// Save upsert the Car in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *CarUpsertOne) Save(ctx context.Context) (*Car, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: CarUpsertOne.Save is not supported by MySQL driver. Use CarUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *CarUpsertOne) SaveX(ctx context.Context) *Car {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
