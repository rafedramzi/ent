...
// Save upsert the Role in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *RoleUpsertOne) Save(ctx context.Context) (*Role, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: RoleUpsertOne.Save is not supported by MySQL driver. Use RoleUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *RoleUpsertOne) SaveX(ctx context.Context) *Role {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
