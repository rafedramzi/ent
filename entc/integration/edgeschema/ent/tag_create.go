...
// Save upsert the Tag in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *TagUpsertOne) Save(ctx context.Context) (*Tag, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: TagUpsertOne.Save is not supported by MySQL driver. Use TagUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *TagUpsertOne) SaveX(ctx context.Context) *Tag {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
