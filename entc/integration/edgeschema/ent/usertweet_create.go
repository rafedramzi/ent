...
// Save upsert the UserTweet in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *UserTweetUpsertOne) Save(ctx context.Context) (*UserTweet, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: UserTweetUpsertOne.Save is not supported by MySQL driver. Use UserTweetUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *UserTweetUpsertOne) SaveX(ctx context.Context) *UserTweet {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
