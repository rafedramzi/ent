...
// Save upsert the Friendship in the database and returns the last inserted record. Will return error on MYSQL dialect.
func (u *FriendshipUpsertOne) Save(ctx context.Context) (*Friendship, error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return nil, errors.New("ent: FriendshipUpsertOne.Save is not supported by MySQL driver. Use FriendshipUpsertOne.Exec instead")
	}
	node, err := u.create.Save(ctx)
	if err != nil {
		return node, err
...
// SaveX calls Save and panics if Save returns an error.
func (u *FriendshipUpsertOne) SaveX(ctx context.Context) *Friendship {
	node, err := u.Save(ctx)
	if err != nil {
		panic(err)
	}
